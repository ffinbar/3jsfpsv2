...
class Colt {
    constructor(player, scene, world, object, body, muzzle, fireFrom, flashObj, fireSound, ricochetSound, decal, decalMat, meshMap, EVERYTHING_ELSE_GROUP, PLAYER_GROUP, listener) {
        this.scene = scene
        this.object = object
        this.player = player
        this.world = world
        ...
        this.adsPosition = new THREE.Vector3(0, -0.125, -0.4);
        this.grabReset = new THREE.Vector3(0.2, -0.2, -0.4);
        this.grabPosition = new THREE.Vector3(0.2, -0.2, -0.4);
        this.body.parent = this;
        this.canFire = true;
        this.fireDelay = 0.5;
        this.recoilMult = 0.02;
        this.recoilMultDefault = 0.02;
    ...
    click() {
        if(this.canFire && !this.timeOut) {
            ...
            this.recoilValueYaw = (((Math.random() - 0.5) * 2) * 0.5) * this.recoilMult
    ...
    rightClick() {
        ...
        if(this.adsConstraint == null) {
            let lhConstraint = new CANNON.PointToPointConstraint(this.player.ragdollLeftHand, new CANNON.Vec3(0, 0, 0), this.body, new CANNON.Vec3(0, 0, 0));
            this.player.ragdollLeftHand.position.copy(this.body.position);
            this.world.addConstraint(lhConstraint);
            this.adsConstraint = lhConstraint;
